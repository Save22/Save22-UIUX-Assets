...
var zoomMethods = {
	init: function(opts) {
		...
		data.zoom = {
			...
			eventDestroying: function() {
				return zoomMethods._eDestroying.apply(that, arguments);
			}
		};
		...
		options.flipbook.
			...
			bind('destroying', data.zoom.eventDestroying);
	...
	_eDestroying: function() {
		...
		var that = this,
			data = this.data().zoom,
			flip = data.opts.flipbook,
			events = ['doubleTap', 'resize', 'zoomIn', 'zoomOut', 'swipeLeft', 'swipeRight'];
		...
		flip.unbind('zooming', data.eventZoom).
			unbind('start', data.eventStart).
			unbind('turning', data.eventTurning).
			unbind('turned', data.eventTurned).
			unbind('destroying', data.eventDestroying);
		...
		if ($.isTouch) {
			...
			this.unbind('click', zoomMethods._tap);
